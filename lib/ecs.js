...
class Stage {
  constructor (processorsProperties, entitiesFacets) {
    ...
    entitiesFacets.forEach(entityFacets => this.engine.createEntity(entityFacets));
  ...
  load () {
    this.processors.forEach(processor => processor.onLoad());
  }
  drop () {
    this.processors.forEach(processor => processor.onDrop());
  }
  step (delta) {
