...
const defaultProcessorProperties = { load () {}, drop () {}, prepare () {}, update () {} };

function createProcessor (engine, properties) {
  const processor = Object.assign(defaultProcessorProperties, properties);
  const facetsAccessors = {
    get (entity, type) { return engine.getFacet(entity, type); },
    set (entity, type, facet) { engine.addFacet(entity, type, facet); }
  };
  return {
    ...
    update (e, delta) { processor.update(facetsAccessors, e, delta); }
  };
}
